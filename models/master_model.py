...
def init_attrs(model, config):
    ...
    model.save_weights(device)
    model.instantiate_mask(device)

    if 'flip' in config['prune_criterion']:
        model.flip_counts = [torch.zeros_like(layer, dtype=torch.float).to(device) 
                            for layer in model.prunable_params]
                            
        model.ema_flip_counts = [torch.zeros_like(layer, dtype=torch.float).to(device) 
                                for layer in model.prunable_params]
    
    if config['prune_criterion'] == 'historical_magnitude':
        model.historical_magnitudes = [torch.zeros_like(layer, dtype=torch.float).to(device)
                                        for layer in model.prunable_params]
...
class MasterModel(nn.Module):
    ...
    def instantiate_mask(self, device):
        self.mask = [torch.ones_like(layer, dtype=torch.bool).to(device) 
                     for layer in self.prunable_params]

    def save_weights(self, device):
        self.saved_weights = [layer.data.detach().clone().to(device) 
                              for layer in self.prunable_params]
    ...
    def update_mask_flipout(self, rate):
        with torch.no_grad():
            flat_mask = torch.cat([layer_mask.view(-1) for layer_mask in self.mask])
